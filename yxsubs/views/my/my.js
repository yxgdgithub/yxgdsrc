		...
		main.ajax({
			...
			"_back":function(res){
				...
				if(res.content.signFlag == 0){
					...
					$("#signOut").hide();
					$("#signIn").css("display","block").show();
				}
				console.log(JSON.stringify(res));
				//判断是否显示音频
				if(res.content.voiceFilePathAndName){
					$("#gdRadio").show();
					var wxAudio = new Wxaudio({
						ele: '#gdRadio',
						title: "",//填写返回数据
						disc: "",//填写返回数据
						src: res.content.voiceFilePathAndName,//填写返回数据
						width: '320px'//自定义宽度
					});
					function play() {
						wxAudio.audioPlay();
				...
				//判断是否展示共读内容
				if(res.content.txtFilePathAndName){
					$(".myReadTitle").load("../../"+res.content.txtFilePathAndName)
					$("#gdPage").show();
					$("#gdNopage").hide();
					//简单音频使用方法，分别定义：id、音频显示文件名、作者、音频文件地址、播放器宽度
				}else{
					var resData = res.content;
					$("#gdNopage").html(resData.errorMessage);
					$("#gdNopage").show();
		...
		function my(){
			main.ajax({
				...
				"_back":function(res){
					...
					var resData = res.content;
					var bookName = resData.bookName;
					var bookSrc = resData.bookSltPath
					var rowNum = resData.rownum;
					var signContinuousDays = resData.signContinuousDays;
					var myHeadImg = resData.headUrl;
					
					$("#myHeadImg").attr("src",myHeadImg);
					$("#bookName").html(bookName);
					$("#bookSltPath").attr("src",bookSrc);
					$("#rownum").html(rowNum);
					$("#signContinuousDays").html(signContinuousDays);
				}
			})
		}
